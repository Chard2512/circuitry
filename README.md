...
A Circuit Maker 2 (Roblox) library that enables savestring building through various abstraction levels, including support for Hardware Description Languages, such as Verilog.

In the context of the game, a "savestring" is a string that represents the data of a save, which can be loaded from and to the Register Tool, enabling the storage of saves and programmable building.

In circuitry, a save is a Module class, which is basically composed of Blocks, Arrays, Wires and Buildings. This library provides you ways to create modules, modify them by adding blocks, connections, move them etc, via code.
